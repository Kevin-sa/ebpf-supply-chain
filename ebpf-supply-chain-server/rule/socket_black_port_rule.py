...
class ScoketBlackPortRule(RuleExecute):
    """
    根据IP地址黑名单判责
    """
    ...
    def __init__(self):
        ...
        self.rule_name = "socket_black_port"
        self.score = 80
        self.black_port_list = [0, 80, 443, 22]

    def do_business(self, package_name: str) -> None:
        try:
            self.logger.info(f"{self.rule_name} package:{package_name}")
            ...
            for result in results:
                self.logger.info(f"{self.rule_name} result port:{result.get('dport', 0)}")
                if result.get("dport", 0) not in self.black_port_list:
